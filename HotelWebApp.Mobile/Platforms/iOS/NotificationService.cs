...
namespace HotelWebApp.Mobile.Services
{
    public partial class NotificationService
    {
        ...
        partial void InitializePlatform()
        {
            ...
            System.Diagnostics.Debug.WriteLine("✅ iOS Notification Center Initialized");
        }
        ...
        public partial async Task<bool> RequestPermissionsAsync()
        {
            try
            {
                ...
                if (error != null)
                {
                    System.Diagnostics.Debug.WriteLine($"❌ iOS Notification Permission Error: {error.LocalizedDescription}");
                    return false;
                }

                System.Diagnostics.Debug.WriteLine($"iOS Notification Permission: {(granted ? "✅ Granted" : "❌ Denied")}");
                return granted;
            }
            catch (Exception ex)
            {
        ...
        public partial async Task ShowNotificationAsync(string title, string message, int notificationId)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"✅ iOS Notification Sent: {title} (ID: {notificationId})");
            }
            catch (Exception ex)
            {
        ...
        public partial async Task CancelNotificationAsync(int notificationId)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"✅ iOS Notification Cancelled (ID: {notificationId})");

                await Task.CompletedTask;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"❌ Error cancelling iOS notification: {ex.Message}");
            }
        }
    }
}